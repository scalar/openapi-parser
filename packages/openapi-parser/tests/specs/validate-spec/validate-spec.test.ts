...
/**
 * This test suite is for validating the Swagger 2.0 specification.
 */
describe('Invalid APIs (Swagger 2.0 specification validation)', () => {
  const invalidFiles = [
    {
      file: 'invalid/invalid-response-code.yaml',
      message:
        'Validation failed. /paths/users/get/responses/888 has an invalid response code (888)',
    },
  ]

  test.skip.each(invalidFiles)('[%s] validate', async (item) => {
    // Exact file path
    const file = `./packages/openapi-parser/tests/specs/validate-spec/${item.file}`
    ...
    expect(result.errors[0].error).toContain(item.message)
  })
  // let tests = [
  //   {
